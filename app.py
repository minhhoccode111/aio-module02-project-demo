...
st.markdown(
    "<h1 style='text-align: center;'>🧑‍💼 Face-Based Employee Check-in System</h1>",
    unsafe_allow_html=True,
)
...
def load_faiss_index():
    ...
    if not os.path.exists("facenet_features.index") or not os.path.exists(
        "facenet_label_map.npy"
    ):
...
def visualize_embeddings(query_embedding=None, matches=None):
    ...
    labels = label_map.copy() if label_map is not None else []
    ...
    if matches:
        for i, (name, similarity, idx) in enumerate(matches):
            if idx < len(colors):  # Ensure we don't go out of bounds
                colors[idx] = (
                    "green" if i == 0 else "orange"
                )  # Top match is green, others orange
                sizes[idx] = 12
                hover_names[idx] = (
    ...
    perplexity = min(
        30, max(5, n_samples - 1)
    )  # Use 5-30 range, but always < n_samples
    ...
    if query_embedding is not None and matches:
        for i, (name, similarity, idx) in enumerate(
            matches[:5]
    ...
    if matches:
        ...
        st.dataframe(
            df_matches.style.highlight_max(subset=["Similarity"], color="lightgreen"),
...
with col1:
    ...
    if not st.session_state.checkin_status:
        st.warning(
            "No employee data available. Please ensure the database is properly set up."
        )
    else:
    ...
    if st.session_state.get("capture_clicked", False):
        ...
        if st.session_state.get("captured_image"):
            with st.spinner("🔍 Finding match..."):
                query_embedding = image_to_feature(
                    st.session_state.captured_image, face_recognition_model
                )
    ...
    if st.session_state.captured_image is not None:
        ...
        if st.session_state.matching_result:
            ...
            visualize_embeddings(
                ...
                [
                    (name, dist, idx)
                    for name, dist, idx in st.session_state.all_matches[:5]
                ],
            )
